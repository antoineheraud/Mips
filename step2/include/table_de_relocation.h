...
typedef enum typez type;

struct table_de_relocation {
	char section[STRLEN];					/*nom de la section, soit .text soit .data*/
	...
	void* psymbol;						/*pointeur vers l'étiquette ou le symbole dans la table des symboles*/
	 			/*l'élément suivant dans la table de relocation*/
};
...
TS erreur_table_de_relocation(LISTE tabreloc, char eti[STRLEN]);
